...
**版型**（寬 1120px，行動裝置摺為單欄）：頂部為健康度燈號＋KPI 速覽（整體進度%、距下個里程碑天數、進行中、待客戶項數），其後依序：里程碑（含時間軸）→ 近期截止｜待客戶事項（雙欄）→ 進行中｜最近完成（雙欄）→ 全部工項明細表。

**健康度燈號**：`dashboard.health: green|amber|red`（選填）＋ `dashboard.health_note`（一句對外說明）。由 PM 手動評估、pm-review 時提示重評——產生器與 AI 均不得自動判定，因為它是對客戶的正式表態。
